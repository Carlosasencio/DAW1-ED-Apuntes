...
- Codigo fuente
- Codigo intermedio u objeto >>> (es un código binario, no es ejecutable)
- Codigo binario >>> (también se puede llamar "código ejecutable")
- Bibliotecas (librerías) >>> (es un código objeto, disponible para un lenguaje de programación y se organiza en distintos archivos)
- Compilar >>> (pasa de codigo fuente a codigo objeto)
- Enlazar (Link) >>> (Vincula el codigo objeto de la aplicacion con el codigo objeto de la biblioteca estandar y de otras bibliotecas)
- Interpretar >>> (leer el codigo fuente y ejecutarlo directamente)
